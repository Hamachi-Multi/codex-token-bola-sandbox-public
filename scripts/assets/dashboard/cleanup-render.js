...
function retentionDeleteConfirmBody() {
  ...
  return `<dl class="cleanup-confirm-summary">
    ...
    ${cleanupConfirmMetric('Delete Through', cutoff)}
...
function allDataDeleteConfirmBody() {
  ...
  return `<dl class="cleanup-confirm-summary">
    ${cleanupConfirmMetric('Rows', compactNumber(rows), exactNumber(rows))}
    ${cleanupConfirmMetric('Delete Through', 'all logs')}
    ${cleanupConfirmMetric('Files', compactNumber(files), exactNumber(files))}
  </dl>
...
function cleanupDetailCutoffMeta() {
  return `<div class="cleanup-detail-meta"><span>Delete through</span><span class="cleanup-detail-meta-value">${esc(cleanupCutoffDateLabel())}</span></div>`;
}
...
function cleanupAffectedFilePageState(files, row = null) {
  const server = ((cleanupDisplayForRow(row) || {}).pagination || null);
  if (server) {
    const total = Math.max(0, Number(server.total_items || 0));
    const pages = Math.max(1, Number(server.total_pages || 0));
    const page = Math.max(1, Number(server.page || 1));
    const pageSize = Math.max(1, Number(server.page_size || CLEANUP_AFFECTED_FILE_PAGE_SIZE));
    const start = (page - 1) * pageSize;
    state.cleanupDetailPage = page;
    return {total, pages, page, start, end: Math.min(total, start + files.length), pageFiles: files};
  }
...
function renderCleanupFileDetail(row, detailRow = row) {
  ...
  const pageState = cleanupAffectedFilePageState(affectedFiles, detailRow);
