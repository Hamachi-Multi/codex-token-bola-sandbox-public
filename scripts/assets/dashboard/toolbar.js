...
function closeToolbarCustomPopover(kind, restoreSelect = true, {restoreFocus = false} = {}) {
  ...
  if (restoreFocus && kind === 'days') {
    document.getElementById('days').focus({ preventScroll: true });
  }
}
...
function handleToolbarCustomInputKeydown(event, commit, kind) {
  if (event.key === 'Enter') {
    ...
    event.preventDefault();
    closeToolbarCustomPopover(kind, true, {restoreFocus: true});
  }
}
