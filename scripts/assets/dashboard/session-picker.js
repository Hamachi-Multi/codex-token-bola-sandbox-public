...
function renderSessionPickerOptions() {
  if (state.sessionOptionsError) {
    ...
    document.getElementById('session-search').setAttribute('aria-activedescendant', '');
    return;
  }
  ...
  document.getElementById('session-options').innerHTML = options.length
    ? options.map((row, index) => {
      ...
      return `<div id="session-option-${index}" class="session-option" role="option" tabindex="-1" aria-selected="${selected ? 'true' : 'false'}" data-session-id="${esc(sessionId)}" data-active="${index === state.sessionActiveIndex ? 'true' : 'false'}">
  ...
  document.getElementById('session-search').setAttribute('aria-activedescendant', options.length ? activeId : '');
}
...
function invalidatePendingSessionOptionLoads() {
...
function chooseSessionFilter(sessionId) {
  invalidatePendingSessionOptionLoads();
...
function handleSessionPickerKeydown(event) {
  ...
  } else if (event.key === 'Tab' && state.sessionFilterOpen) {
    closeSessionPicker();
  }
}
...
function bindSessionPickerControls() {
  ...
  document.addEventListener('focusin', event => {
    if (state.sessionFilterOpen && !event.target.closest('#session-picker')) closeSessionPicker();
  });
}
